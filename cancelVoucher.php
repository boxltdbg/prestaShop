...
$authorization = "Authorization: Bearer " . $auth_json['access_token'];
	...
	curl_setopt($cancelation, CURLOPT_USERAGENT, 'BoxNow-PrestaShop-Module/2.0');
	...
	// BoxNow returns 2xx on a successful cancel (with or without a body). The old
	// check relied on an empty body, which almost never matched, so the local
	// voucher state was never reset and the "Create Vouchers" UI never returned.
	if ($cancelation_http_status >= 200 && $cancelation_http_status < 300) {
		...
		$totalvouchers = max(0, (int) $boxnow['vouchers'] - 1);
		// Remove the cancelled parcel id from the stored list.
		...
		$msg = is_array($cancelation_json) && isset($cancelation_json['code']) ? $cancelation_json['code'] : 'Unknown error';
		echo 'Error (HTTP ' . (int) $cancelation_http_status . '): ' . $msg;
	}
