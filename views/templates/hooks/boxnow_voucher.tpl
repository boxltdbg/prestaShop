...
<div id="boxnowCard" class="boxnow card">
	...
	<div class="card-body">
		...
		{if $boxnow_vouchers == 0}
			...
			<div class="form-group">
					{l s='Vouchers for this order' mod='boxnow'} ({l s='Max' mod='boxnow'} {$total_order_products}):
				</label>
...
$(function () {
	...
	$('.boxnow-cancel-voucher').on('click', function () {
		...
		const linkCancel = '{$boxnow_cancelvoucher_link}';
		if (orderId > 0) {
			$.post(linkCancel, { order_id: orderId, voucher_number: voucher }, function (data) {
				if (data === 'success') {
	...
	$('#create_vouchers').on('click', function () {
		...
		const linkCreate = '{$boxnow_createvoucher_link}';
